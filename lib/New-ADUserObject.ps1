function New-ADUserObject {
 ...
 process {
  ...
  if (-not$obj) {
   ...
   $shortTerm = if ( ($_.empid -match '\d{7,}') -or ($shortTermTypes -match $_.jobType) ) { $true }
   if ($shortTerm) {
    ...
    Write-Host ('{0},{1}, Account Expiration set: {2}' -f $MyInvocation.MyCommand.Name, $samid, $accountExpirationDate)
   ...
   #TODO
   # if ( $BargUnitId ) {  Set-ADUser $samid -add @{extensionAttribute1 = "$bargUnitID" } }
   ...
   "smtp:$samid@mail.chicousd.org",
   "smtp:$samid@chicousd.mail.onmicrosoft.com"
